...
class CarliniL2:
    ...
    def __init__(self, targeted=True, search_steps=None, max_steps=None, cuda=False, debug=False, num_classes=14, word_mode=False):
        self.debug = debug
    ...
    def _optimize(self, optimizer, model, input_var, modifier_var, target_var, scale_const_var, input_token=None):
        ...
        if self.mask is None:
            ...
            for i in range(input_adv.size(0)):
                # i = 批次内的样本序号 bi，锁定当前处理的单个样本
                new_word_list = []
                add_start = self.batch_info['add_start'][i]
                add_end = self.batch_info['add_end'][i]
                similar_wv=self.similar_wv

                for j in range(add_start, add_end):
                    if self.word_mode:
                        # ===================== 【核心修复：4行代码完美适配批次索引】 =====================
                        # ✅ 修改1：二维索引 → [批次内样本i][token序号j]，精准定位当前样本的词汇起始位
                        if self.batch_info['start_mark'][i][j] == 1:
                            # ✅ 修改2：取当前样本i的start_mark一维数组，判断边界
                            if j == len(self.batch_info['start_mark'][i]) - 1:
                                k = j + 1
                            else:
                                # ✅ 修改3：对当前样本i的一维start_mark调用index，找下一个词汇起始位
                                k = self.batch_info['start_mark'][i].index(1, j+1)
                            # ======================================================================
                            similar_wv = similar_wv.view(similar_wv.shape[0], -1)
                            new_placeholder = input_adv[i, j:k].data.view(1, -1)
                            temp_place = new_placeholder.expand_as(similar_wv)
                            new_dist = torch.norm(temp_place - similar_wv.data, 2, -1)
                            _, new_word = torch.min(new_dist, 0)
                            print("new_word")
                            new_word_list.append(new_word.item())
                            print("no error")
                            input_adv.data[i, j:k] = self.itereated_var.data[i, j:k] = \
                                similar_wv[new_word.item()].data.view(-1, input_adv.shape[2])
                            del temp_place
                    else:
